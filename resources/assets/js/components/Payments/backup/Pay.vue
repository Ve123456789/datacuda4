        ...
        methods: {
            payToken(){
                ...
                createToken().then((data) => { 
                    console.log(this.userData,"1234 user data");
                    const user =JSON.parse(window.localStorage.getItem('user'));
               
                  
                    const userDetails=this.userData;
           console.log(userDetails,"this is the user data");
                    axios
                    .post('/api/planpay', {userDetails,token: data.token, subscriptionPlanId: window.localStorage.getItem('selected_plan_id')})
                    .then(({ data }) => {
