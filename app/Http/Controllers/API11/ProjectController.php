...
class ProjectController extends Controller
{
    ...
    public function get_project_data(Request $request)
    {
        ...
        return response()->json(['status' => 200, 'token' => request('auth_token'), 'message' => 'Success', 'project_medias' => $project['medias']
                ,'project_data'=>$project,'path'=>$project['image_path'],'project_size' => $size]
            );
    }
    ...
    public function sendprojectlink(Request $request)
    {
        ...
        if ($sm_model->save()) {
            ...
            $mail_details = [
                ...
                'data' => $share_link,
                'project_data' => array('file_count'=>$file_count,'medias'=> $project_data['medias'])
            ];
    ...
    public function ProjectSendByMail(Request $request)
    {
        ...
        $sm_model->description = json_encode($request->description);
        $sm_model->amount = json_encode($request->amount);
        $sm_model->amount_type = json_encode($request->amount_type);
        ...
        $loopamount = 0;
        foreach($request->amount as $m_amount): $run_amount = $loopamount; $loopamount  = $run_amount + $m_amount; endforeach;
